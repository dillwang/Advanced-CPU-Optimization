...
| `Item is incompatible with the array type` / `CONST is not an unpacked array, but is in an unpacked array context` | Verilator applies `'{default:}` only ONE level deep, and the array has more dimensions | nest one `'{default:}` per unpacked dimension — but see the next row, yosys-slang wants the opposite |
| `invalid target type 'logic' for assignment pattern` / `assignment pattern target type cannot be deduced` | yosys-slang already recursed, so your nested `'{default:}` lands on a leaf. It wants the FLAT form Verilator rejects | no single literal works. `` `ifdef VERILATOR `` around `FILL2`/`FILL3` macros in the shared header; nested for Verilator, flat otherwise |
| `Unknown verilator comment: '/*verilator ...*/'` | a comment line begins with the word Verilator, which makes it a metacomment | reword so that word is not first after `//` |
